...
def data_cleaning(**context):
    ...
    filtered_df: pd.DataFrame = df[(df['miles'] < 26.2) & (df['location'] != 'Riverside, CT')]
    return filtered_df.to_dict(orient='list')


def data_analysis(**context):
    ti: TaskInstance = context["ti"]
    data: Dict[str, Any] = ti.xcom_pull(task_ids='data_cleaning_task', key='return_value')
...
with DAG(
    ...
    data_analysis_task = PythonOperator(task_id='data_analysis_task', python_callable=data_analysis)
